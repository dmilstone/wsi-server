  ...
  <div class="brand">
    <div class="brand-copy">
      ...
      <div class="brand-current-image">
        ...
        <div id="selected-name">Sample_Slide_Long_Name_ABCDEFG.vsi</div>
      </div>
    </div>
  </div>
  <div id="tools-tray" class="tools-tray"><div class="viewer-toolbar" role="toolbar">
    <button id="home-view" class="toolbar-button" type="button" data-tooltip="Home" aria-label="Home"></button>
...
(() => {
  ...
  const ctx = document.querySelector(".brand-current-image").getBoundingClientRect();
...
for (const viewport of viewports) {
    ...
    assert.ok(measured.ctx.width <= 360 + 1, `${viewport.name} CURRENT IMAGE stays bounded`);
